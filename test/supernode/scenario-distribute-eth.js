...
export async function distributeEth(supernodeAddress, txOptions) {
  // Get contracts
  ...
  const supernodeFee = fees.supernodeEthFee_;
  const nodeOperatorFee = fees.nodeOperatorEthFee_;

  const totalShares = await supernode.getTotalEthShares();
  ...
  //
  // console.log('Unclaimed per share: ' + await supernode.unclaimedEthPerShare());
